...
import { Bridge, type NDEFRecord, type TagDetectedEvent, type WriteSuccessEvent, type WriteErrorEvent } from '@aspect/webview-bridge'
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    async function startScan() {
      ...
      // 设置事件监听
      removeTagListener = Bridge.nfc.onTagDetected((data: TagDetectedEvent) => {
    ...
    async function writeTextToTag() {
      ...
      // 设置事件监听
      removeWriteSuccessListener = Bridge.nfc.onWriteSuccess((_data: WriteSuccessEvent) => {
        emit('log', 'success', 'NFC 标签写入成功')
        removeWriteSuccessListener?.()
        removeWriteErrorListener?.()
      })

      removeWriteErrorListener = Bridge.nfc.onWriteError((data: WriteErrorEvent) => {
      ...
      <div class="section">
        ...
        {detectedRecords.value.length > 0 && (
          <div style={{ marginBottom: '12px' }}>
            ...
            <CellGroup inset>
              {detectedRecords.value.map((record: NDEFRecord, index: number) => (
                <Cell
                  key={index}
                  title={`记录 ${index + 1}`}
