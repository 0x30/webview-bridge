...
public class ContactsModule: NSObject, BridgeModule {
    ...
    private func requestPermission(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
    ...
    private func getContacts(
        ...
        guard status == .authorized else {
            callback(.failure(BridgeError(code: .permissionDenied, message: "未获得通讯录权限")))
            return
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                ...
                callback(.failure(BridgeError(code: .internalError, message: "读取联系人失败: \(error.localizedDescription)")))
            }
        }
    }
    ...
    private func pickContact(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
    ...
    private func getContact(
        ...
        do {
            ...
            callback(.failure(BridgeError(code: .internalError, message: "获取联系人失败: \(error.localizedDescription)")))
        }
    }
    ...
    private func createContact(
        ...
        if let phones = params["phones"]?.arrayValue as? [[String: Any]] {
            contact.phoneNumbers = phones.compactMap { phone in
                guard let number = phone["number"] as? String else { return nil }
                let label = phone["label"] as? String ?? CNLabelPhoneNumberMobile
        ...
        do {
            ...
            callback(.failure(BridgeError(code: .internalError, message: "保存联系人失败: \(error.localizedDescription)")))
        }
    }
    ...
    private func contactToDictionary(_ contact: CNContact, includePhoto: Bool = false) -> [String: Any] {
        var dict: [String: Any] = [
            "identifier": contact.identifier,
