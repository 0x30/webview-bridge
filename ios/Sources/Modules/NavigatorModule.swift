...
public class PageStackManager {
    ...
    /// 页面栈
    private var pageStack:
        [(
            info: PageInfo, bridge: WebViewBridge,
    ...
    /// 页面创建回调（用于自定义页面样式）
    public var pageFactory:
        ((String, String?, [String: Any]?) -> UIViewController)?
    ...
    private func generatePageId() -> String {
        pageIdCounter += 1
        return
            "page_\(pageIdCounter)_\(Int(Date().timeIntervalSince1970 * 1000))"
    }
    ...
    /// 根据ID获取页面
    public func getPage(byId id: String) -> (
        info: PageInfo, bridge: WebViewBridge, viewController: UIViewController
    )? {
        return pageStack.first { $0.info.id == id }
    }

    /// 注册根页面（首次加载的页面）
    public func registerRootPage(
        ...
        if let navigatorModule = bridge.getModule("Navigator")
            as? NavigatorModule
        {
            navigatorModule.setCurrentPageId(pageId)
        }
    ...
    public func push(
        ...
        guard let navController = rootNavigationController else {
            completion(
                .failure(BridgeError.unknown("NavigationController 未设置"))
            )
            return
        }
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let config =
                self.webViewConfigFactory?() ?? WKWebViewConfiguration()
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            for _ in 0..<popCount {
                if let popped = self.pageStack.popLast() {
                    ...
                    // 发送页面销毁事件
                    popped.bridge.sendEvent(
                        "Navigator.PageDestroyed",
                        data: popped.info.dictionary
                    )
                }
            }
            ...
            if popCount == 1 {
                ...
                let targetVC = navController.viewControllers[
                    navController.viewControllers.count - popCount - 1
                ]
    ...
    public func close(
        ...
        guard
            let pageIndex = pageStack.firstIndex(where: { $0.info.id == pageId }
            )
        else {
            completion(.failure(BridgeError.unknown("页面不存在")))
            return
        }

        let page = pageStack[pageIndex]

        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }

            // 如果是栈顶页面，直接 pop
            if pageIndex == self.pageStack.count - 1 {
                // 通知前一个页面（如果有）
                if pageIndex > 0 {
                    let previousPage = self.pageStack[pageIndex - 1]
                    if let resultData = result {
                        previousPage.bridge.sendEvent(
                            "Navigator.Result",
                            data: [
                                "from": page.info.dictionary,
                                "result": resultData,
                            ]
                        )
                    }
                ...
                // 如果不是栈顶，需要先 pop 到该页面，再 pop 掉它
                // 这种情况比较复杂，暂时不支持
                completion(.failure(BridgeError.unknown("只能关闭栈顶页面，请先关闭后续页面")))
            }
        }
    }
}
...
public class NavigatorModule: NSObject, BridgeModule {
    ...
    private func push(
        ...
        PageStackManager.shared.push(
            ...
            case .failure(let error):
                callback(
                    .failure(BridgeError.unknown(error.localizedDescription))
                )
            }
        }
    }
    ...
    private func postMessage(
        ...
        let sourcePageId =
            currentPageId ?? PageStackManager.shared.currentPage?.info.id ?? ""
    ...
    private func close(
        ...
        guard let pageId = currentPageId else {
            callback(.failure(BridgeError.unknown("当前页面 ID 未设置")))
            return
        }
