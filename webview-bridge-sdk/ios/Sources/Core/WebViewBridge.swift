...
public struct BridgeConfiguration {
    ...
    /// 带自定义 Scheme 的配置
    public static func withURLScheme(_ scheme: URLSchemeConfiguration) -> BridgeConfiguration {
...
public class WebViewBridge: NSObject {
    ...
    ///   - configuration: 配置
    public init(webView: WKWebView, configuration: BridgeConfiguration = .default) {
    ...
    public static func create(
        ...
        if let urlScheme = configuration.urlScheme {
            ...
            webViewConfig.setURLSchemeHandler(schemeHandler, forURLScheme: urlScheme.scheme)
        }
        ...
        let bridge = WebViewBridge(webView: webView, configuration: configuration)
    ...
    private func setupURLSchemeHandler() {
        guard let webView = webView,
              let urlScheme = configuration.urlScheme else { return }
    ...
    private func handleMessage(_ messageBody: Any) {
        ...
        module.handleRequest(
            ...
            case .success(let data):
                self?.sendResponse(.success(callbackId: request.callbackId, data: data))
    ...
    public func sendResponse(_ response: BridgeResponse) {
        ...
        let script = "window.__bridgeCallback && window.__bridgeCallback('\(escapedJson)')"
    ...
    public func sendEvent(_ event: BridgeEvent) {
        ...
        let script = "window.__bridgeEvent && window.__bridgeEvent('\(escapedJson)')"
    ...
    @objc private func appWillEnterForeground() {
        sendEvent(BridgeEvent(event: "App.Foreground", data: ["timestamp": Date().timeIntervalSince1970]))
    ...
    @objc private func appDidEnterBackground() {
        sendEvent(BridgeEvent(event: "App.Background", data: ["timestamp": Date().timeIntervalSince1970]))
