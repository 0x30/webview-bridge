...
public class SystemModule: BridgeModule {
            ...
            if inApp && (url.scheme == "http" || url.scheme == "https") {
                ...
                UIApplication.shared.open(url, options: [:]) { success in
                    if success {
                        ...
                        callback(.failure(BridgeError(code: .internalError, message: "无法打开 URL")))
                    }
                }
            }
        }
    ...
    private func openInSafariViewController(url: URL, callback: @escaping (Result<Any?, BridgeError>) -> Void) {
    ...
    private func share(
        ...
        if let text = params.getString("text") {
            items.append(text)
        }
        
        if let urlString = params.getString("url"),
           let url = URL(string: urlString) {
            items.append(url)
        }
        
        if let imageBase64 = params.getString("image"),
    ...
    private func getLanguage(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
    ...
    private func getTimezone(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
    ...
    private func getSafeArea(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
        DispatchQueue.main.async {
    ...
    private func rateApp(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
        DispatchQueue.main.async {
            if #available(iOS 14.0, *) {
                if let scene = UIApplication.shared.connectedScenes.first(where: { $0.activationState == .foregroundActive }) as? UIWindowScene {
                    ...
                    callback(.failure(BridgeError(code: .internalError, message: "无法获取窗口场景")))
                }
            } else {
    ...
    private func openAppStore(
        ...
        UIApplication.shared.open(url, options: [:]) { success in
            if success {
                ...
                callback(.failure(BridgeError(code: .internalError, message: "无法打开 App Store")))
            }
        }
    }
    ...
    private func getColorScheme(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
        DispatchQueue.main.async {
    ...
    private func setBrightness(
        ...
        guard let brightness = params.getDouble("brightness") else {
    ...
    private func getBrightness(callback: @escaping (Result<Any?, BridgeError>) -> Void) {
        DispatchQueue.main.async {
    ...
    private func is24HourFormat() -> Bool {
        ...
        return !timeString.contains(formatter.amSymbol ?? "AM") && !timeString.contains(formatter.pmSymbol ?? "PM")
    }
