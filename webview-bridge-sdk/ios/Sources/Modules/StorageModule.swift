...
public class StorageModule: BridgeModule {
    ...
    private func get(
        ...
        let securityLevelString =
            params.getString("securityLevel") ?? "standard"
        let securityLevel =
            StorageSecurityLevel(rawValue: securityLevelString) ?? .standard
    ...
    private func set(
        ...
        case .secure:
            let accessibilityString =
                params.getString("accessibility") ?? "afterFirstUnlock"
            let accessibility =
                KeychainAccessibility(rawValue: accessibilityString)
                ?? .afterFirstUnlock
    ...
    private func setMultiple(
        ...
        guard let items = params["items"]?.value as? [String: String] else {
    ...
    private func keychainClearAll() -> Int {
        ...
        var result: AnyObject?
        let countStatus = SecItemCopyMatching(
            countQuery as CFDictionary,
            &result
        )
