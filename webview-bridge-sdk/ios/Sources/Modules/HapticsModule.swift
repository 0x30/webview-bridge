...
public class HapticsModule: BridgeModule {
    ...
    public let methods = [
        "Impact", "Notification", "Selection", "Vibrate", "IsSupported",
    ]
    ...
    private func impact(
        ...
        let intensity = (params["intensity"]?.value as? Double) ?? 1.0
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if #available(iOS 13.0, *) {
                generator.impactOccurred(
                    intensity: CGFloat(min(1.0, max(0.0, intensity)))
                )
            } else {
                generator.impactOccurred()
            }
            ...
            for (index, duration) in pattern.enumerated() {
                if index % 2 == 0 {
                    ...
                    DispatchQueue.main.asyncAfter(
                        deadline: .now() + Double(
                            self?.sumUpTo(pattern, index: index) ?? 0
                        ) / 1000.0
                    ) {
